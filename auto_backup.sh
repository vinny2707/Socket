# Log File Directory
if [ S# -ne 3 ]; then
	echo "Usage: $ <source_dir> <backup_dir> <max_backups>"
	exit 1
fi
...
log "START:  Backup form $SOURCE_DIR  to $BACKUP_FILE"
START_TIME=$(date + %s)
...
if [ $? -eq 0 ]; then
	...
	log "FAIL: Compresstion failure with $BACKUP_NAME. TIME: $ELAPSED s."
fi
